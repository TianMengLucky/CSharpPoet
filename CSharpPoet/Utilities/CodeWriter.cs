...
public class CodeWriter : IndentedTextWriter
{
    ...
    private static string? GetSeparator(ICSharpMember member) => (member as IHasSeparator)?.Separator;
    ...
    public void WriteMembers<T>(IList<T> members) where T : ICSharpMember
    {
        ...
        for (var i = 0; i < members.Count; i++)
        {
            ...
            if (separator != null && i < members.Count - 1)
            {
                ...
                if (GetSeparator(next) == null && next is not CSharpBlankLine) Write(separator);
            }
        }
    }
...
#pragma warning disable CA1815
    ...
    public readonly struct MultilineCommentScope : IDisposable
    {
        ...
        internal MultilineCommentScope(CodeWriter writer)
        {
            ...
            if (_writer._isInMultilineComment) throw new ArgumentException("Can't enter a multiline comment twice");
